...
from starlette_admin import (
    ...
    EnumField, IntegerField, FloatField, TextAreaField, JSONField, DropDown,
)
...
# ---------------------------------------------------------------------------
# Mixin pour filtrer les vues par rôle
# ---------------------------------------------------------------------------

class _RoleFilteredView(ModelView):
    """Surcharge find_all et count pour restreindre à un rôle."""
    _role_filter: Any = None  # valeur MongoDB pour le filtre "role"
    ...
    async def find_all(
        ...
        **kwargs: Any,
        ...
        if not isinstance(where, dict):
            where = {}
        where["role"] = self._role_filter
        return await super().find_all(request, skip, limit, where, order_by, **kwargs)
...
# ---------------------------------------------------------------------------
# Champs communs pour les vues utilisateurs
# ---------------------------------------------------------------------------
...
_USER_EXCLUDE_CREATE = ["id", "created_at", "last_active", "banned_at"]
_USER_EXCLUDE_EDIT   = ["id", "created_at", "last_active", "banned_at"]
...
def build_admin_views(admin: Admin) -> None:
    ...
    # Clients
    # ------------------------------------------------------------------
    class ClientView(_RoleFilteredView):
        _role_filter = "client"
        fields = _USER_FIELDS
        column_list = ["email", "full_name", "is_active", "is_banned", "created_at"]
        column_sortable_list = ["email", "full_name", "created_at"]
        column_searchable_list = ["email", "full_name"]
        exclude_fields_from_create = _USER_EXCLUDE_CREATE
        exclude_fields_from_edit   = _USER_EXCLUDE_EDIT

    # ------------------------------------------------------------------
    # Prestataires (utilisateurs avec role=provider)
    # ------------------------------------------------------------------
    class ProviderUserView(_RoleFilteredView):
        _role_filter = "provider"
        fields = _USER_FIELDS
        column_list = ["email", "full_name", "is_active", "is_banned", "created_at"]
        column_sortable_list = ["email", "full_name", "created_at"]
        column_searchable_list = ["email", "full_name"]
    ...
    # Administrateurs
    # ------------------------------------------------------------------
    class AdminUserView(_RoleFilteredView):
        _role_filter = "admin"
        fields = _USER_FIELDS
        column_list = ["email", "full_name", "is_active", "created_at"]
        column_sortable_list = ["email", "full_name", "created_at"]
        column_searchable_list = ["email", "full_name"]
        exclude_fields_from_create = _USER_EXCLUDE_CREATE
        exclude_fields_from_edit   = _USER_EXCLUDE_EDIT
    ...
    admin.add_view(DropDown(
        ...
        icon="fa fa-users",
        views=[
            ClientView(User, label="Clients"),
            ProviderUserView(User, label="Prestataires", identity="provider-user"),
            AdminUserView(User, label="Administrateurs", identity="admin-user"),
        ],
    ))
...
def build_admin_views(admin: Admin) -> None:
    ...
    class UserView(ModelView):
        ...
        exclude_fields_from_create = [
            "id", "created_at", "last_active", "banned_at",
        ]
